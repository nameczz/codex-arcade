...
一个可直接加入 Codex 的本地游戏插件市场。目前包含两款无需网络、无需第三方游戏素材的 MCP App：
...
安装后新建一个 Codex 任务，输入 `打开打砖块` 或 `打开贪吃蛇`。新任务能确保 Codex 载入最新的技能和 MCP 工具。
...
支持时，两款游戏都可以向 Codex 请求画中画或全屏；不支持时会在界面内给出提示，不影响内联游戏。
